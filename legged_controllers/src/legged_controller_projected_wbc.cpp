...
#include "legged_controllers/legged_controller_projected_wbc.h"
...
namespace legged
{
bool LeggedControllerProjectedWBC::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& controller_nh)
{
  // Initialize OCS2
  ...
  // Whole body control
  wbc_ = std::make_shared<ProjectedWBC>(task_file, *legged_interface_, ee_kinematics, verbose);
...
void LeggedControllerProjectedWBC::starting(const ros::Time& time)
{
  // Initial state
...
void LeggedControllerProjectedWBC::update(const ros::Time& time, const ros::Duration& period)
{
  // State Estimate
...
LeggedControllerProjectedWBC::~LeggedControllerProjectedWBC()
{
...
void LeggedControllerProjectedWBC::setupLeggedInterface(const std::string& task_file, const std::string& urdf_file,
...
void LeggedControllerProjectedWBC::setupMpc()
{
...
void LeggedControllerProjectedWBC::setupMrt()
{
...
void LeggedControllerProjectedWBC::setupStateEstimate(LeggedInterface& legged_interface,
...
void LeggedCheaterControllerProjectedWBC::setupStateEstimate(LeggedInterface& legged_interface,
                                                 const std::vector<HybridJointHandle>& hybrid_joint_handles,
                                                 const std::vector<ContactSensorHandle>& contact_sensor_handles,
                                                 const hardware_interface::ImuSensorHandle& imu_sensor_handle)
{
...
PLUGINLIB_EXPORT_CLASS(legged::LeggedControllerProjectedWBC, controller_interface::ControllerBase)
PLUGINLIB_EXPORT_CLASS(legged::LeggedCheaterControllerProjectedWBC, controller_interface::ControllerBase)
